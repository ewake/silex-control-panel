...
date_default_timezone_set($app['config_timezone']);
//ini_set('date.timezone', $app['timezone']);

//http://wordpress.stackexchange.com/a/5352
if (in_array(PHP_SAPI, ['cli', 'cli-server'])) {
	if(array_key_exists(PHP_SAPI, $app['config_php_sapi'])) {
		putenv("HTTP_HOST=".$app['config_php_sapi'][PHP_SAPI]['HTTP_HOST']);
		putenv("SERVER_NAME=".$app['config_php_sapi'][PHP_SAPI]['SERVER_NAME']);
		putenv("REQUEST_URI=".$app['config_php_sapi'][PHP_SAPI]['REQUEST_URI']);
		putenv("REQUEST_METHOD=".$app['config_php_sapi'][PHP_SAPI]['REQUEST_METHOD']);
	}
}